...
const MyInterestBungae: React.FC = () => {
    ...
    //   }, [sortOrder, dispatch]);
      ...
      const deleteToggleInterest = async (bungaeId: number, interested: boolean) => {
        try {
            const response = await dispatch(deleteInterest({ bungaeId: bungaeId, userId: userId })).unwrap();
            console.log(`Successfully toggled interest for post ${bungaeId}`);
        }
        catch (error) {
            Swal.fire({
      ...
      const handleButtonClick = (category: string) => {
        if (category === "moim") {
          ...
          navigate("/interestedBungae");  //frontend 측의 url
        }
      };
      ...
      return (
        <PostListContainer>
      <ButtonGroup>
        <Button selected={false} onClick={() => handleButtonClick("moim")}>
          모임
        </Button>
        <Button selected={true} onClick={() => handleButtonClick("bungae")}>
          번개
        </Button>
      </ButtonGroup>
        ...
        {visiblePosts.map((post) => (
          <PostCard key={post.id} onClick={() => handleCardClick(post.id)}>
            ...
            <HeartIcon
              ...
              onClick={(e) => {
                e.stopPropagation();
                {post.interested ? deleteToggleInterest(post.id, post.interested || false) : handleToggleInterest(post.id, post.interested || false)}
              }}
            />
            <PostInfo>
...
export default MyInterestBungae;