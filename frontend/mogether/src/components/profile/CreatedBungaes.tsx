...
const CreatedBungae: React.FC = () => {
    ...
    //   }, [sortOrder, dispatch]);
      ...
      const handleDelteInterest = async (bungaeId: number, interested: boolean) => {
        try {
          const response = await dispatch(deleteInterest({ bungaeId: bungaeId, userId: userId })).unwrap();
          console.log(`Successfully delete interest for post ${bungaeId}`);
        } catch (error) {
          Swal.fire({
            icon: 'error',
      ...
      const handleButtonClick = (category: string) => {
        if (category === "moim") {
          navigate("/createdMoim");
        } else if (category === "bungae") {
          navigate("/createdBungae");
        }
      };
